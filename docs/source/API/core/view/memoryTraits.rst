...
Note that the type alias ``Kokkos::MemoryManaged`` is deprecated. ``Kokkos::MemoryTraits<>`` is the default memory trait of a ``View``. There is no practical use for mentioning it explicitly. Also, ``Kokkos::MemoryRandomAccess`` is planned to be defined as ``Kokkos::MemoryTraits<Kokkos::RandomAccess>`` alone. Since, it is a trait that is applicable to both managed and unmanaged Views. See |ProgrammingGuide|_ for practical uses of ``Kokkos::MemoryUnamanged``/``Kokkos::MemoryTraits<Kokkos::Unmanaged>`` and ``Kokkos::MemoryTraits<Kokkos::RandomAccess>``. 
